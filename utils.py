...
def stack_segments(vertebrae):
    """Extract the segments from each vertebra and stack them"""
    return np.array([x['segments'] for x in vertebrae])
...
def choose_sagittal(annotation):
    ...
    for expert in annotation['Annotation'].values():
        indices.append(stack_segments(expert)[..., 1].mean())
...
def draw_annotation(image, annotation):
    ...
    for color, (expert, points) in zip(colors, annotation['Annotation'].items()):
        ...
        points = stack_segments(points)[..., [0, 2]]
